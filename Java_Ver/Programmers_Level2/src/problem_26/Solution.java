...
*      - 15일 이후부터 29일 이후까지 필요한 밀가루는 15톤이므로 더 이상의 공급은 필요 없습니다. 따라서 총 2회의 밀가루를 공급받으면 됩니다.
*/

/*
 *  #. 나의 풀이법.
 *    - priority_queue를 어떻게 사용해야 할지 몰랐음.
 *    
 *  #. 풀이
 *    - 공급 횟수의 최소의 값을 구해야한다. 우선순위 큐(내림차순) 사용한다. 
 *    - stock = 0이면, 항상 최대의 값(pq.top())을 공급한다.
 *    - Greedy 방식.
...
public class Solution {
	...
	public int solution(int stock, int[] dates, int[] supplies, int k) {
        int answer = 0;
        //
        //1. 내림 차순의 우선순위 큐를 만든다.
        PriorityQueue<Integer> pq = new PriorityQueue<>(Collections.reverseOrder());
        int i = 0;
        for(int d = 0; d < k; d++) {
        	if(i < dates.length && d == dates[i]) {
        		pq.offer(supplies[i]);
        		i++;
        	}
