...
bool myCompare(const pair<double, int> &a, const pair<double, int> &b)
{
    if (a.first == b.first)
...
vector<int> solution(int N, vector<int> stages) 
{
    ...
    for (int i = 1; i <= N; i++)
    {
        ...
        if (fail_cnt == 0)
            ...
            fail.push_back({ (double)fail_cnt / player_cnt, i });
        player_cnt -= fail_cnt;
    }

    sort(fail.begin(), fail.end(), myCompare);
...
int main()
{
    ...
    printResult(solution(5, { 2,1,2,6,2,4,3,3 }));
    printResult(solution(4, { 4,4,4,4 }));
    printResult(solution(5, { 6, 6, 6, 6, 6, 6, 6 }));
    return 0;
}
